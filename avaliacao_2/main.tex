...
de sinais. Enquanto a representação no tempo descreve \emph{quando} os eventos ocorrem,
a representação em frequência revela \emph{de que} componentes oscilatórias o sinal é
...
\item \textbf{Deslocamento no tempo.} Um atraso não altera a magnitude do espectro,
apenas acrescenta uma fase linear $-\omega n_d$. Isso mostra que a informação sobre a
\emph{posição} do sinal está contida na fase, e não na magnitude.
\end{alineas}
...
mesma DFT da Equação~\eqref{eq:dft}. O resultado é idêntico ao do cálculo direto ---
apenas se obtém muito mais rápido.
...
mostra $X(e^{j\omega})$ com sinal, sobreposto às amostras da FFT --- a coincidência é
total. O painel central mostra a magnitude, com o primeiro zero em
...
As propriedades da TFTD --- em especial a da convolução e sua dual, a da multiplicação
--- mostraram-se o elo entre a teoria e a prática da filtragem digital. A ligação com a
